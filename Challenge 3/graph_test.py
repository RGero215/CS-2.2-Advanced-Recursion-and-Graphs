...
class GraphTest(unittest.TestCase):
    ...
    def test_depth_first_search(self):
        ...
        jessie = graph.add_vertex('Jessie Pichardo')
        joel = graph.add_vertex('Joel Pichardo')
        eduardo = graph.add_vertex('Eduardo Geronimo')
        junior = graph.add_vertex('Junior Dominguez')
        ...
        graph.add_edge('Ramon Geronimo', 'Jessie Pichardo')
        graph.add_edge('Jessie Pichardo', 'Joel Pichardo')
        graph.add_edge('Eduardo Geronimo', 'Junior Dominguez')
        graph.add_edge('Junior Dominguez', 'Joel Pichardo')
...
# (5,6)
        ...
        path = graph.depth_first_search('Ramon Geronimo', 'Joel Pichardo')
        self.assertEqual(path, [ramon, jessie, joel])
        print(path)
        path_2 = graph.depth_first_search('Ramon Geronimo', 'Jessie Pichardo', visit=[])
        self.assertEqual(path_2, [ramon, jessie])
        print(path_2)
        path_3 = graph.depth_first_search('Eduardo Geronimo', 'Joel Pichardo', visit=[])
        print(path_3)
        # self.assertEqual(path_3, [eduardo, junior])
        # path = g.depth_first_search(1,5, visit=[])
        # print(path)
        # g.read_graph_from_file('graph_data.txt')
        # print('Type test: ', g.directed)
